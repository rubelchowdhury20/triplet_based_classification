...
class Train():
	def __init__(self, train_loader, val_loader, model, criterion, optimizer, epoch):
	...
	def train(self, batch_hard=False):
		...
		for batch_index, batch in tqdm(enumerate(self.train_loader)):
			batch_images = batch[0].to(config.DEVICE)
	...
	def validate(self, batch_hard=False):
		...
		with torch.no_grad():
			for batch_index, batch in tqdm(enumerate(self.val_loader)):
				batch_images = batch[0].to(config.DEVICE)
				labels = batch[1].to(config.DEVICE)
				embeddings = self.model(batch_images)
				if not batch_hard:
				...
				if batch_index % 10 == 0:
					print('\nValidation Progress:\t Average loss: {:.4f}\t'
